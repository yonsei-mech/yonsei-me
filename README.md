...
파일을 고친 뒤에는 저장소에 반영(커밋·푸시)되면 서버가 자동으로 다시 배포합니다
(보통 몇 분). 이 과정은 운영 담당자가 도와드립니다.
...
- 배포: **Cafe24**(서버 빌드 + SSH 전환 — `.github/workflows/deploy.yml`)
...
- **참조 데이터·문서**(연혁·교수진·교직원·교과목 3종·동아리·연구실·장학금md) → **GitHub
  ...
  그 커밋이 자동 배포를 태워 몇 분 뒤 반영됩니다. 콘솔에 노출할 항목·폼은
...
정리돼 있습니다. 로컬은 `.env.local`, 배포는 서버의 `/etc/yonsei_me/env` 에 넣습니다.

- `main` 에 푸시하면 `.github/workflows/deploy.yml` 이 SSH 로 Cafe24 서버의
  `bin/deploy.sh` 를 호출합니다. **빌드는 서버에서** 합니다 — 운영 비밀값을 서버 밖으로
  내보내지 않기 위해서입니다. 저장소 비밀값 `DEPLOY_SSH_KEY` 가 없으면 배포를 건너뜁니다.
- `NEXT_PUBLIC_` 으로 시작하는 값은 **빌드 시점에 구워집니다** — 바꾸면 재시작이 아니라
  재배포(재빌드)가 필요합니다.
