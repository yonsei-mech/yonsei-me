 ...
 *
 * ⚠️ **<head> 에서** 동기 실행해야 한다 — <body> 가 만들어지기 전에 클래스가 있어야 body 의 첫
 * 레이아웃부터 body::before 가 마진을 진다. 처음엔 body 첫머리(PerfLiteScript 바로 뒤)에 두었는데
 * 배포본 실측(slow-4G 10회)에서 6회·2회 시프트가 남았다: PerfLiteScript 의 WebGL 판정(새 탭 첫
 * 방문, 수십 ms) 뒤 파서가 쉬는 틈에 클래스 없는 레이아웃이 한 번 돈다. head 시점 부착은 0/10.
 * head 의 인라인 스크립트는 앞선 렌더 차단 CSS 를 기다리지만, 그 CSS 가 오기 전엔 어차피 아무것도
 * 그려지지 않으므로 손해가 없다(바로 위 webfonts-rest 로더와 같은 처지).
